...
namespace Microsoft.WindowsMobile.Samples.Location
{
    ...
    public class Gps
    {
        ...
        //private bool forceStop = false; //Added by Ðonny
        ...
        private void WaitForGpsEvents()
        {
            lock (this)
            {
                ...
                while (listening)
                {
                    //if (forceStop) listening = false;//Added by Ðonny (maybe unnecessary)
                    int obj = WaitForMultipleObjects(3, handles, 0, 1000);
                    if (obj != waitFailed)
                    {
                        switch (obj)
                        {
