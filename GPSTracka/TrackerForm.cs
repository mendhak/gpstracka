...
namespace GPSTracka {
    ...
    public partial class TrackerForm : Form { //renamed from GPSTracka to TrackerForm - old name puzzled the designer
#region API
        ...
        /// <param name="pvDevice">[in] Specifies the device. Must be a valid LPWSTR device name, for example, "COM1:". The meaning is determined by the <paramref name="Flags"/> parameter.</param>
        ...
        public static extern IntPtr SetPowerRequirement(String pvDevice, DevicePowerState DeviceState, int DeviceFlags, IntPtr pvSystemState, int StateFlags);
...
#region Status 
        ...
        void Status(string message, int slot) {
            if (stsStatus.InvokeRequired) {
                try {
                    stsStatus.BeginInvoke(new Action<string, int>(Status), message, slot);
                } catch (ObjectDisposedException) { }
            } else {
...
#region Sentence
        ...
        void LogSentence(Gps.GpsSentenceEventArgs e) {
            ...
            if (!LogSentenceOnStack) {
                ...
                try { Application.DoEvents(); } finally { LogSentenceOnStack = false; }
            }
        }
        void gps_GpsSentence(Gps.GpsProvider sender, Gps.GpsSentenceEventArgs e) {
            ...
            if (LogEverything) {//Verbose (to textbox)
            ...
            if (AdvancedConfig.NmeaLog) {//NMEA file logging
                if (NMEAFileName == null)
                    NMEAFileName = System.IO.Path.Combine(AdvancedConfig.LogFileLocation, DateTime.Now.ToString("yyyyMMdd_HHmmss", System.Globalization.CultureInfo.InvariantCulture) + ".nmea");
        ...
        void gps_Position(Gps.GpsProvider sender, Gps.GpsPositionEventArgs e) {
            ...
            if (pos.Longitude.HasValue && pos.Latitude.HasValue){
                ...
                if (!positionLogged) {
                    ...
                    if (LastPositionA == null) {//Stop-start point unknown (immediatelly after start)  
                        LogThisPoint = true;
                    } else {
                        ...
                        if (distance < AdvancedConfig.MinimalDistance) { //Point is to near
                            LastPositionC = pos;//Remember it
                            LastTimeC = time; //And remember the time
                        } else {//Points are far enough
                            ...
                            if (LastPositionC != null && time != LastTimeA && LastTimeA != LastTimeC /*&& distAC >= AdvancedConfig.MinimalDistance*/) { //Commented-out - always log last pause point
                                WriteToFile(
                                    LastTimeC,
        ...
        void gps_GpsCommState(Gps.GpsProvider sender, Gps.GpsState state) {
            switch (state) {
                /*case States.AutoDiscovery:
                    ...
                    break;*/
                case GPSTracka.Gps.GpsState.Openning:
                    if (LogEverything) WriteToTextbox(Properties.Resources.Opening);
                    Status(Properties.Resources.status_Openning);
                    break;
...
#region Satellites
        private void GPS_Sattelite(GPSTracka.Gps.GpsProvider sender, GPSTracka.Gps.GpsSatteliteEventArgs e) {
            if(statistic==null && this.InvokeRequired) {this.BeginInvoke(new Gps.GpsSatteliteEventHandler(GPS_Sattelite),sender,e);return;}
            int cnt=0;
            foreach (var sat in e.Sattelites)
                if (sat.SignalToNoiseRatio != 0) ++cnt;
            if (statistic != null)
                statistic.CurrentSats = cnt;
            else
                lblSattelites.Text = cnt.ToString();
        }
        /// <summary>Instance of window showing GPS sattelites</summary>
        SattelitesView sattelitesWindow = null;
        private void mniSattelites_Click(object sender, EventArgs e) {
            if (sattelitesWindow == null) {
                sattelitesWindow = new SattelitesView(GPS);
                sattelitesWindow.Closed += new EventHandler(sattelitesWindow_Closed);
            }
            sattelitesWindow.Show();
        }
        private void sattelitesWindow_Closed(object sender, EventArgs e) {
            sattelitesWindow.Closed -= sattelitesWindow_Closed;
            sattelitesWindow = null;
        }
#endregion
#endregion
        ...
        private void WriteToCSVFile(DateTime time, double latitude, double longitude, double altitude) {
            ...
            using (IDisposable stream = System.IO.File.Open(currentFileName, FileMode.Open, FileAccess.ReadWrite, FileShare.Read),
                ...
                foreach (char ch in AdvancedConfig.CSVFields) {
                    ...
                    if (AdvancedConfig.CSVQualifierUsage == CSVQualifierUsage.Always || (fieldval.Contains(AdvancedConfig.CSVSeparator.ToString()) && AdvancedConfig.CSVQualifierUsage == CSVQualifierUsage.AsNeeded)) {
...
#region Control GPS running
        ...
        private void SetTimerEnabled(Timer timer, bool enabled) {
            ...
            if (this.InvokeRequired) {//Required as timer started in bad thread may not work
        ...
        private void GpsStopAndWait() {
            try {
                while (GPS.State ==  GPSTracka.Gps.GpsState.Openning) {
...
#region Settings
        ...
        private void saveMenuItem_Click(object sender, EventArgs e) {
            ...
            if (sattelitesWindow != null) sattelitesWindow.Close();//To prevent it from using different GPS provider
            HookGps(false);
            gps_instance = null;
...
#region Statistic
        ...
        private void OnPoint(DateTime time, GpsPoint gpsPoint, bool LogThisPoint) {
            ...
            if (LogThisPoint) {//This point is logged, count it
                var delta = gpsPoint - statistic.CurrentPos;//km
                statistic.SumLength += delta;
                statistic.CurrentPos = gpsPoint;
                statistic.PointsTotal++;
            } else {//This point is not logged, pause
                statistic.PauseTime += time - stat_lastLogTime;
            }
            stat_lastLogTime = time;
            if (stat_lastEle == null) {//Initialize altitude if necessary
                stat_lastEle = gpsPoint.Altitude;
                return;
            }
            if (LogThisPoint && gpsPoint.Altitude.HasValue) {//Elevation
        ...
        private class GpsStatistics {
            ...
            /// <summary>Current number of sattelites in view</summary>
            ...
            public void ShowValues(ValueKind vk) {
                ...
                if ((vk & ValueKind.Sats) != 0)
                    form.lblSattelites.Text = CurrentSats.ToString();
            }
...
internal struct GpsPoint {
    ...
    public GpsPoint(decimal latitude, decimal longitude, decimal? altitude):this(){
        Latitude = latitude;
        Longitude = longitude;
        Altitude = altitude;
    }
    ...
    #region http://www.experts-exchange.com/Database/GIS_and_GPS/Q_23651016.html
